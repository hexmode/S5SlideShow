...
class S5SlideShow {
  ...
  } else { $this->pageContent = $this->sArticle->getContent();
  }
		$this->attr = [];
		if ( is_array( $attr ) ) {
	...
	function genSlideFile( $printPageSize = false ) {
		...
		foreach ( $this->slides as $slide ) {
			...
			} else { 				$slides_html .= "<div class='slide$slide0 notitle'>$c</div>\n";
			}
			$slide0 = "";
		}
	...
	static function genStyle( $skin, $print = false ) {
		...
		foreach ( S5SlideShowHooks::$styles as $k => $file ) {
			...
			if ( $title->exists() ) {
				$a = new Article( $title );
				$c = $a->getContent();
			} else { 				$c = @file_get_contents( "$dir/" . str_replace( '$skin', $skin, $file ) );
			}
  ...
  } else { $html = "<div style='float: $attr[float]; margin: " . ( $attr['float'] == 'left' ? '0 1em 1em 0' : '0 0 0 1em' ) . "'>$html</div>";
  }
		return $html;
	}
